...
class Kip {
    ...
    constructor(repoPath ='.') {
        this.repoPath = path.join(repoPath, '.kip');
        this.objectsPath = path.join(this.repoPath, 'objects'); //.kip/objects
        this.headPath = path.join(this.repoPath, 'HEAD'); //.kip/HEAD
        this.indexPath = path.join(this.repoPath, 'index'); //.kip/index
    }
    ...
    async init() {
        ...
        try {
            ...
            console.log("Already initialized the .kip folder");
        }
    }
    ...
    async add(fileToBeAdded) {
        ...
        const newFileHashedObjectPath = path.join(this.objectsPath, fileHash); // .kip/objects/abc123
    ...
    async commit(message) {
        ...
        await fs.writeFile(this.headPath, commitHash); // update head to point to new commit 
        await fs.writeFile(this.indexPath, JSON.stringify([])); // clea the staging area
...
program.command('show <commitHash>').action(async (commitHash) => {
    const kip = new Kip();
    await kip.showCommitDiff(commitHash);
});
